...
def main():
    ...
    Two types of image augmentation are applied. One is on a normal
    image whose image name starts with 1 and another is on a flipped
    ...
    type of specie. We check the first value of the image path
    ...
    for bird_specie in species:
        ...
        if not os.path.isdir(augmented_image_folder):
            print(f"Error: Directory not found: {augmented_image_folder}")
            continue

        source_images = os.listdir(augmented_image_folder)
        if len(source_images) == 0:
            print(f"Warning: No images found in {augmented_image_folder}")
            continue
